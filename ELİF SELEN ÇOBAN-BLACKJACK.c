...
main()
{
	...
	while (1) {
		...
		for (int i = 0; i < 52; i++)
		{
			...
			for (int j = -1; j < i; j++)
			{
				if (random[i] == random[j]) //bir öncekileRle ayný olmamalý
				{
					i--;
					break;
		...
		while (1) {
			...
			if (soru == 'e') {
				...
				if (deger[sayac] == 1 && oyuncutoplam < 12) {//Oyuncu as çekerse ve elinin toplamý 12 den küçükse
					while (1) {
						printf("\nAs cektiniz 11 olarak kullanmak ister misiniz ?  Evet(e) Hayir(h)");
						scanf_s(" %c", &assoru);
						if (assoru == 'e' || assoru == 'E') {
			...
			if (oyuncutoplam > 21) {//Oyuncu 21 i geçerse kart çekme döngüsünü sonlandýrýyor
				break;
			}

		
		}
		if (oyuncutoplam > 21) {// Oyuncu 21 i geçerse kaybeder masa kart çekmeden oyun sonlanýr oyuncuya tekrar oynamak istiyor musun diye sorulur
			printf("\n\t\t\t\t\tKAYBETTINIZ");
			kaybetme++;
			...
			printf("Masanin Kartlari:%s\t%s", card[random[0]], card[random[1]]);//Masanýn 17 den büyük olduðu durumlarda da elindekileri yazmasý için döngü dýþýnda
		...
		if (masatoplam > 21) {//Masa 21 i geçerse oyuncu kazanýr
			printf("\n\t\t\t\t\tKAZANDINIZ"); kazanma++;
		}
